...
export const SECTION_META: Record<string, { label: string; description: string }> = {
  ...
  update: { label: "更新", description: "Auto-update settings and release channel" },
  agents: { label: "代理", description: "Agent configurations, models, and identities" },
  auth: { label: "认证", description: "API keys and authentication profiles" },
  channels: {
    label: "通道",
    description: "消息通道（Telegram、Discord、Slack 等）",
  },
  messages: { label: "消息", description: "消息处理与路由设置" },
  commands: { label: "命令", description: "Custom slash commands" },
  hooks: { label: "钩子", description: "Webhooks and event hooks" },
  ...
  wizard: { label: "设置向导", description: "Setup wizard state and history" },
  // Additional sections
  meta: { label: "元数据", description: "网关元数据与版本信息" },
  logging: { label: "日志", description: "Log levels and output configuration" },
    ...
    return html`
      <div class="config-empty">
        ...
        <div class="config-empty__text">
          ${searchQuery ? `没有设置项匹配“${searchQuery}”` : "该分区下没有设置项"}
        </div>
      </div>
    `;
  }
